...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fuel_type_id > 0 && $price_per_liter >= 0) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT id FROM fuel_pricing WHERE station_id = ? AND fuel_type_id = ? AND is_active = 1");
            ...
            log_activity($pdo, $me['id'], 'Update Fuel Pricing', "Updated fuel pricing for fuel type ID: $fuel_type_id to ₱$price_per_liter/L");
        } catch (Exception $e) {
...
try {
    if ($role === 'superadmin') {
        // For superadmin, show all fuel types with nozzle counts
        $stmt = $pdo->query(
            "SELECT 
                ft.id,
                ft.name,
             ...
             FROM fuel_types ft
             LEFT JOIN fuel_pricing fp ON ft.id = fp.fuel_type_id AND fp.is_active = 1
             LEFT JOIN nozzles n ON ft.id = n.fuel_type_id
             GROUP BY ft.id, ft.name
             ORDER BY ft.name"
        );
        $fuel_types = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } else {
        // For admin/manager, show only their station with nozzle details
        $stmt = $pdo->prepare(
            "SELECT 
                ft.id,
                ft.name,
                COALESCE(fp.price_per_liter, 0) as price_per_liter,
                COUNT(DISTINCT n.id) as nozzle_count,
                GROUP_CONCAT(DISTINCT CONCAT('Nozzle ', n.nozzle_number) SEPARATOR ', ') as nozzles
             FROM fuel_types ft
             LEFT JOIN fuel_pricing fp ON ft.id = fp.fuel_type_id AND fp.station_id = ? AND fp.is_active = 1
             LEFT JOIN nozzles n ON ft.id = n.fuel_type_id AND n.pump_id IN (
                SELECT id FROM fuel_pumps WHERE station_id = ?
             )
             GROUP BY ft.id, ft.name
             ORDER BY ft.name"
        );
              ...
              <tr>
                ...
                <td>
                  <div class="nozzle-info">
                    <?php 
                    if ($fuel['nozzle_count'] > 0) {
                      echo "Nozzles: ";
                      if (isset($fuel['nozzles'])) {
                        echo htmlspecialchars($fuel['nozzles']);
                      } else {
                        echo htmlspecialchars($fuel['stations'] ?? '-');
                      }
                    } else {
                      echo "No nozzles";
                    }
