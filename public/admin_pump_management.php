...
try {
    ...
    if ($selected_station) {
        $stmt = $pdo->prepare("
            SELECT fp.id, fp.pump_number, fp.fuel_type_id, fp.status, ft.name as fuel_type_name, s.name as station_name
            FROM fuel_pumps fp
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_pump') {
        ...
        if (!$pump_number || !$fuel_type_id) {
            $msg = "❌ Error: Pump number and fuel type are required.";
        } else {
            try {
            ...
            <table class="pump-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-primary" onclick="openEditPumpModal(<?php echo $pump['id']; ?>, '<?php echo htmlspecialchars($pump['pump_number']); ?>', <?php echo $pump['fuel_type_id']; ?>, '<?php echo $pump['status']; ?>')">
...
window.onclick = function(event) {
    let modal = document.getElementById('addPumpModal');
    if (event.target === modal) {
        modal.classList.remove('show');
    }
    modal = document.getElementById('editPumpModal');
    if (event.target === modal) {
        modal.classList.remove('show');
    }
    modal = document.getElementById('deletePumpModal');
    if (event.target === modal) {
        modal.classList.remove('show');
    }
}
