            ...
            if (empty($user_id)) {
                ...
                try {
                    ...
                    if (!$userInfo) {
                        ...
                        // STATION RESTRICTION: Admin can only reset password for staff in their assigned station
                        if ($me['role'] === 'admin' || $me['role'] === 'Admin') {
                            if ($userInfo['station_id'] != $me['station_id']) {
                                $notice = 'You can only reset passwords for staff in your assigned station.';
                            } elseif (in_array(strtolower($userInfo['role']), ['admin', 'manager'])) {
                                $notice = 'You can only reset passwords for staff users.';
                            } else {
...
try {
    ...
    // STATION FILTER: If admin (not super admin), show only users from their station
    if ($me['role'] === 'admin' || $me['role'] === 'Admin') {
        // Admin can only see staff in their assigned station
        $sql .= " AND u.station_id = " . intval($me['station_id']) . " AND u.role NOT IN ('admin', 'Admin', 'manager')";
    }
                    ...
                    <tr data-role="<?php echo htmlspecialchars($user['role']); ?>" 
                        ...
                        <td>
                            <div class="action-buttons">
                                ...
                                <button class="action-btn reset" onclick="console.log('Reset button clicked'); showResetModal(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars($user['name']); ?>', '<?php echo htmlspecialchars($user['username']); ?>')" title="Reset Password">
...
function showResetModal(userId, userName, username) {
    console.log('showResetModal called with:', { userId, userName, username });
    ...
    console.log('Modal elements updated, showing modal');
